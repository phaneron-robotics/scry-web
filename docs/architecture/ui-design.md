...
App Launch
    ...
    ▼
┌──────────────┐
│ Scry (Chat)  │ ← Start destination. If no saved robot, Fleets opens instead.
└──────┬───────┘
       │
       │   (bottom nav — 6 tabs, swap freely)
       │
┌──────┴─────────────────────────────────────────────────────────────────┐
│  Fleets  ←→  Robot  ←→  Scry (hero)  ←→  ROS  ←→  Viz  ←→  Settings    │
└────────────────────────────────────────────────────────────────────────┘
                                            │
                                            ▼  (deep-links from chat prose)
                                ┌──────────────────────┐
                                │  Viz section pinned  │
                                │  scry://viz?section= │
                                │  ...&topic=...       │
