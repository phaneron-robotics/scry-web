...
```
First launch → Scry tab (empty state)
    │
    ├─ "Use Ollama (Free, Local)"  ─→ Settings → Ollama base URL + model
    ├─ "Use Claude (Recommended)"  ─→ Settings → Anthropic API key
    ├─ "Use OpenAI"                ─→ Settings → OpenAI API key
    └─ "Use Gemini (Free tier)"    ─→ Settings → Google AI API key
```
